...
export const SecurityAssistant: React.FC<Props> = ({ currentStatus, currentServer, userTier, onUpgrade, onServerSelect, allServers }) => {
  ...
  return (
    <div className="max-w-4xl mx-auto h-[calc(100vh-160px)] flex flex-col gap-6 relative">
      {userTier === 'free' && (
        <div className="absolute inset-0 z-10 flex flex-col items-center justify-center bg-black/80 backdrop-blur-md rounded-3xl border border-white/5">
          ...
          <h2 className="text-3xl font-light text-white mb-3 tracking-tight text-center">Nova AI Assistant</h2>
      ...
      <div className={`flex-1 overflow-y-auto space-y-6 pr-2 ${userTier === 'free' ? 'opacity-20 pointer-events-none' : ''}`}>
        ...
        {alerts.length > 0 && (
          <div className="space-y-3">
            ...
            {alerts.map((alert, idx) => (
              <div key={idx} className={`border rounded-3xl p-5 flex items-start gap-4 shadow-lg ${
        ...
        {/* Security Tip Banner */}
        {tip && (
          <div className="bg-[#0A0A0A] border border-white/10 rounded-3xl p-6 flex items-start gap-4 shadow-lg">
            ...
            <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[85%] flex gap-4 ${m.role === 'user' ? 'flex-row-reverse' : ''}`}>
                <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${
                  m.role === 'user' ? 'bg-white text-black' : 'bg-[#0A0A0A] border border-white/10 text-white'
                }`}>
                  {m.role === 'user' ? <User className="w-5 h-5" /> : <Bot className="w-5 h-5" />}
                </div>
                <div className={`p-5 rounded-3xl text-sm leading-relaxed font-light ${
                  m.role === 'user' ? 'bg-white text-black rounded-tr-sm' : 'bg-[#0A0A0A] border border-white/5 text-gray-200 rounded-tl-sm'
                }`}>
                  {m.content}
                </div>
              </div>
      ...
      {/* Input Area */}
      <div className={`p-2 bg-[#0A0A0A] border border-white/10 rounded-full shadow-2xl ${userTier === 'free' ? 'opacity-20 pointer-events-none' : ''}`}>
        <div className="flex gap-2 items-center">
          <input 
            ...
            className="flex-1 bg-transparent border-none focus:ring-0 text-sm placeholder-gray-600 text-white px-6 py-3 font-light outline-none"
            disabled={userTier === 'free'}
          />
          <button 
            onClick={handleSend}
            disabled={isLoading || !input.trim() || userTier === 'free'}
            className="p-3.5 bg-white text-black rounded-full hover:bg-gray-200 transition-colors disabled:opacity-50 disabled:hover:bg-white"
          >
