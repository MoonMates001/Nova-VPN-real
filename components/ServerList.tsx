    ...
    <div className="max-w-4xl mx-auto space-y-8 pb-20">
      ...
      {bestServer && !searchTerm && activeRegion === 'All' && (
        <div 
          ...
          className="group relative bg-[#0A0A0A] border border-white/10 rounded-[2rem] p-8 md:p-10 cursor-pointer overflow-hidden transition-all duration-500 hover:border-white/30 hover:shadow-[0_0_40px_rgba(255,255,255,0.05)]"
        >
          ...
          <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
            <div className="flex flex-col items-center md:items-start text-center md:text-left">
              ...
              <h3 className="text-3xl md:text-4xl font-light text-white mb-3 tracking-tight">Quick Connect</h3>
      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 pt-4">
        <div>
          <h2 className="text-3xl font-light text-white tracking-tight mb-1">Global Network</h2>
          <p className="text-gray-500 text-sm font-light">Choose from 6,500+ ultra-fast servers</p>
        </div>
      ...
      <div className="relative group">
        ...
        <input 
          ...
          className="w-full bg-[#050505] border border-white/10 rounded-2xl py-4 pl-14 pr-12 focus:outline-none focus:border-white/30 transition-all text-white font-light placeholder:text-gray-600"
          value={searchTerm}
      ...
      {processedServers.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {processedServers.map(server => {
            ...
            <div
              ...
              className={`flex items-center gap-5 p-6 rounded-3xl border transition-all text-left cursor-pointer group relative overflow-hidden animate-in fade-in zoom-in-95 duration-300 ${
                selectedId === server.id 
                  ? 'bg-white/5 border-white/30 ring-1 ring-white/10' 
                  : isLocked
                    ? 'bg-[#050505] border-white/5 hover:border-white/10 opacity-60'
            ...
            >
              ...
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-1">
                  <h4 className={`font-medium tracking-tight text-lg transition-colors ${selectedId === server.id ? 'text-white' : 'text-gray-200'}`}>
                    {server.name}
                  </h4>
                  {server.isPremium && (
                ...
                <p className="text-xs text-gray-500 font-light">{server.country}</p>
              </div>
                  ...
                  <button
                    ...
                    className={`p-3.5 rounded-2xl transition-all duration-300 shadow-lg active:scale-95 ${
                      selectedId === server.id 
                        ...
                        : 'bg-[#0A0A0A] text-white border border-white/10 group-hover:bg-white group-hover:text-black group-hover:border-transparent'
                    }`}
                    title="Connect Now"
                  >
                    <Power className="w-5 h-5" />
        ...
        <div className="flex flex-col items-center justify-center py-24 bg-[#050505] border border-dashed border-white/10 rounded-3xl text-center">
          <div className="p-5 bg-[#0A0A0A] border border-white/5 rounded-full mb-6">
            <Globe2 className="w-10 h-10 text-gray-600" />
          </div>
          <h3 className="text-xl font-light tracking-tight text-white mb-2">No servers found</h3>
          <p className="text-sm text-gray-500 max-w-xs px-4 font-light">
